...
export class ApprovalService {
  ...
  async createCategory(createDto: CreateApprovalCategoryDto, companyId: string) {
    ...
    // Create default route template if provided
    if (createDto.default_route && createDto.default_route.length > 0) {
      const routeTemplate = await this.prisma.approval_route_template.create({
        data: {
          tenant_id: '', // This should be provided from context
          category_id: category.id,
          name: `${category.name} Default Route`,
          description: `Default approval route for ${category.name}`,
          is_default: true,
          is_active: true
        }
      });
  ...
  async updateCategory(id: string, updateDto: UpdateApprovalCategoryDto, companyId: string) {
    ...
    if (updateDto.default_route) {
      ...
      if (updateDto.default_route.length > 0) {
        await this.prisma.approval_route_template.create({
          data: {
            tenant_id: '', // This should be provided from context
            category_id: id,
            name: `${updatedCategory.name} Route`,
            description: `Approval route for ${updatedCategory.name}`,
            is_default: true,
            is_active: true
          }
        });
  ...
  async createDraft(createDto: CreateApprovalDraftDto, requesterId: string) {
    ...
    if (createDto.custom_route && createDto.custom_route.length > 0) {
      ...
      // Create route stages
      for (const step of createDto.custom_route) {
        await this.prisma.approval_route_stage.create({
          data: {
            route_id: route.id,
            type: step.step_type || 'APPROVAL',
            mode: 'SEQUENTIAL',
            order_index: step.step_order || 1,
            label: step.instructions || 'Review and approve',
            required_count: 1
          }
        });
      }
